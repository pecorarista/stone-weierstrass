...
\footnotetext{\pronunciation{Raabe}{de}{{\primarystress}ʁaːbə}{ラーベ}}
...
\footnotetext{\pronunciation{Abel}{no}{\primarystress\`{ɑ}ːbəl}{アーベル}}
...
\begin{align}
  ...
  &= (1 - x) \sum_{j = 0}^{n - 1} (s_j - s) x^j \mathlabel{abel}
\end{align}
が成り立つ．

\(x \in (-1, 1)\)において\(\sum_{k = 0}^n a_k x^k \)が\(f\)に収束するので，任意の\(\varepsilon > 0\)に対して，ある\(N \in \mathbb{N}\)が存在して，\(n \geq N\)ならば任意の\(x \in (-1, 1)\)で
\begin{align}
  \lvert f(x)  - \sum_{k = 0}^n a_k x^k\rvert \leq \frac{\varepsilon}{2} \mathlabel{abel2}
\end{align}
が成り立つ．
\mathref{abel}，\mathref{abel2}より
\begin{align*}
    \lvert f(x) - s \rvert
